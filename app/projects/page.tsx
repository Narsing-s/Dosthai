...
const CONTEXT_COOKIE = 'dosthai-project-context';
const MAX_CONTEXT = 3500;

function setProjectContext(project: Project | null) {
  if (!project) {
    document.cookie = `${CONTEXT_COOKIE}=; Max-Age=0; Path=/; SameSite=Lax`;
    return;
  }
  const context = `${project.name}\n${project.instructions}`.trim().slice(0, MAX_CONTEXT);
  document.cookie = `${CONTEXT_COOKIE}=${encodeURIComponent(context)}; Max-Age=2592000; Path=/; SameSite=Lax`;
}
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    try {
      ...
      setProjects(loaded);
      const activeId = localStorage.getItem(`${KEY}-active`);
      const active = loaded.find((project: Project) => project.id === activeId) || loaded[0];
      if (active) {
  ...
  function selectProject(project: Project) {
    ...
    localStorage.setItem(`${KEY}-active`, project.id);
    setProjectContext(project);
  }

  function updateInstructions(value: string) {
    if (!active) return;
    const next = projects.map((p) => p.id === active.id ? { ...p, instructions: value.slice(0, MAX_CONTEXT) } : p);
    save(next);
    const updated = next.find((p) => p.id === active.id);
    if (updated) setProjectContext(updated);
  }

  function createProject() {
    const value = name.trim();
    if (!value) return;
    const project = { id: crypto.randomUUID(), name: value, instructions: instructions.trim().slice(0, MAX_CONTEXT), createdAt: new Date().toISOString() };
