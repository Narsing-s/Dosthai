...
export async function POST(request: Request) {
  ...
  catch (error) { if (error instanceof DOMException && error.name === 'AbortError') return NextResponse.json({ error: 'Agent request was cancelled.' }, { status: 499 }); const messageText = error instanceof Error ? error.message : 'Agent request failed.'; const status = /not configured/i.test(messageText) ? 503 : /timed out/i.test(messageText) ? 504 : /too many/i.test(messageText) ? 429 : 502; return NextResponse.json({ error: messageText }, { status, headers: { 'cache-control': 'no-store' } }); }
}
